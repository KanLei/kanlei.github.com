...
上面的代码会输出这样的错误，*protocol 'Animal' can only be used as a generic constraint because it has Self or associated type requirements* 因为 `Swift` 是一门类型安全的语言，而当我们使用 `Animal` 时却无法指定它的 `associatedtype`，这就导致编译器无法在编译时刻得到具体的类型信息，拒绝通过编译。

想要解决上面这个问题，我们可以使用一点小技巧，向下面这样：
